...
enum FifoStatus : uint8_t {
	FIFO_DATA  = (uint8_t)0x00, // The FIFO contains data and available locations
	FIFO_EMPTY = (uint8_t)0x01, // The FIFO is empty
	FIFO_FULL  = (uint8_t)0x02, // The FIFO is full
	FIFO_ERROR = (uint8_t)0x03  // Impossible state: FIFO cannot be empty and full at the same time
};
