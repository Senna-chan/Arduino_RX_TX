...
auto narrow(From from) {
	if constexpr(std::is_void_v<To>) {
		return impl::narrow_t<From>{from};
	} else {
		return impl::raw_narrow<To>(from);
	}
}

uint16_t minValue = 1000;
uint16_t maxValue = 2000;
uint16_t midValue = 1500;
...
uint16_t parseADCChannel(const uint16_t raw_channel, const uint16_t minValue, const uint16_t midValue, const uint16_t maxValue, const uint16_t deadZone, const uint16_t offset)
{
    uint16_t result = 0;
    if (midValue != 0) {
        if (between(raw_channel, narrow<uint16_t>(midValue - deadZone), narrow<uint16_t>(midValue + deadZone)))
        {
            result = 1500;
        }
        else
        {
            ...
            {
                result = map(raw_channel, minValue, narrow<uint16_t>(midValue - deadZone), minValue, midValue);
            }
            else
            {
                result = map(raw_channel, narrow<uint16_t>(midValue + deadZone), maxValue, midValue, maxValue);
            }
        }
    }
    else
    {
        ...
        {
            result = map(raw_channel, minValue, narrow<uint16_t>(maxValue - deadZone), minValue, maxValue);
        }
    }
    result += offset;
...
uint16_t parseRCChannel(const uint8_t channel_number, const channelConfigs *channel_settings, const rateLimitConfigStruct *rateLimitSettings, const outputEnableStruct *outputEnableSettings, const bool reverse_channel, const uint16_t *adc_values, const uint32_t IO_bits, const uint16_t *AUX_Serial_Channels)
{
    ...
    else if (io1t == CTYPE_IO && io2t == CTYPE_IO) // Multi IO mapped
    {
        ...
        {
            printf("CH%d invalid config. IO1 and IO2 both high\n", channel_number);
        }
    }
    else // Invalid config
    {
        printf("CH Mapping for %d invalid. IOType1 %s Type2 %s\n", channel_number, channel_types_str[io1t], channel_types_str[io2t]); // SPAM
    }
    ...
    if (channel_settings->endPoints.min != minValue || channel_settings->endPoints.max != maxValue)
    {
        returnValue = map(returnValue, minValue, maxValue, channel_settings->endPoints.min, channel_settings->endPoints.max);
    }
...
uint16_t checkRateLimit(const uint8_t channel_number, const rateLimitConfigStruct *rateLimitSettings, const bool centeredStick, const uint16_t *adc_values, const uint32_t IO_bits, const uint16_t *AUX_Serial_Channels, uint16_t channelValue)
{
    ...
    if (rateLimitSettings->outputToRateLimit == channel_number)
    {
        ...
        if (io1t != CTYPE_NONE && io2t == CTYPE_NONE) // Single channel mapped
        {
            ...
            case CTYPE_ADC:
            {
                limitScale = narrow<uint8_t>(adc_values[io1] / 10);
            }
            break;
            ...
            case CTYPE_AUX_SERIAL:
            {
                ...
                {
                    limitScale = narrow<uint8_t>(aux_value / 10);
                }
                else
                {
