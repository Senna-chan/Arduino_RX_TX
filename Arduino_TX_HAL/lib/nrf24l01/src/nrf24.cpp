...
GPIO_TypeDef  *nRF_cePort;
GPIO_TypeDef  *nRF_csnPort;
uint16_t      nRF_cePin;
uint16_t      nRF_csnPin;
...
void nRF24_DumpConfig(void) {
    ...
    nRF24_printf("[0x%02X] 0x%02X MASK:%03b CRC:%02b PWR:%s MODE:P%s\r\n",
            nRF24_REG_CONFIG,
            i,
            i >> 4,
            (i & 0x0c) >> 2,
    ...
    nRF24_printf("[0x%02X] 0x%02X ENAA: ",nRF24_REG_EN_AA,i);
    for (j = 0; j < 6; j++) {
        nRF24_printf("[P%1u%s]%s",j,
                (i & (1 << j)) ? "+" : "-",
                (j == 5) ? "\r\n" : " "
            );
    }
    ...
    nRF24_printf("[0x%02X] 0x%02X EN_RXADDR: ",nRF24_REG_EN_RXADDR,i);
    for (j = 0; j < 6; j++) {
        nRF24_printf("[P%1u%s]%s",j,
                (i & (1 << j)) ? "+" : "-",
                (j == 5) ? "\r\n" : " "
    ...
    aw = (i & 0x03) + 2;
    nRF24_printf("[0x%02X] 0x%02X EN_RXADDR=%06b (address width = %u)\r\n",nRF24_REG_SETUP_AW,i,i & 0x03,aw);
    // SETUP_RETR
    i = nRF24_ReadReg(nRF24_REG_SETUP_RETR);
    nRF24_printf("[0x%02X] 0x%02X ARD=%04b ARC=%04b (retr.delay=%uus, count=%u)\r\n",
    ...
    nRF24_printf("[0x%02X] 0x%02X (%.3uGHz)\r\n",nRF24_REG_RF_CH,i,2400 + i);
    // RF_SETUP
    i = nRF24_ReadReg(nRF24_REG_RF_SETUP);
    nRF24_printf("[0x%02X] 0x%02X CONT_WAVE:%s PLL_LOCK:%s DataRate=",
            nRF24_REG_RF_SETUP,
            i,
            (i & 0x80) ? "ON" : "OFF",
            (i & 0x80) ? "ON" : "OFF"
    ...
    nRF24_printf("[0x%02X] 0x%02X IRQ:%03b RX_PIPE:%u TX_FULL:%s\r\n",
            nRF24_REG_STATUS,
            i,
            (i & 0x70) >> 4,
            (i & 0x0E) >> 1,
    ...
    nRF24_printf("[0x%02X] 0x%02X PLOS_CNT=%u ARC_CNT=%u\r\n",nRF24_REG_OBSERVE_TX,i,i >> 4,i & 0x0F);
    // RPD
    i = nRF24_ReadReg(nRF24_REG_RPD);
    nRF24_printf("[0x%02X] 0x%02X RPD=%s\r\n",nRF24_REG_RPD,i,(i & 0x01) ? "YES" : "NO");
    // RX_ADDR_P0
    nRF24_ReadMBReg(nRF24_REG_RX_ADDR_P0,buf,aw);
    nRF24_printf("[0x%02X] RX_ADDR_P0 \"",nRF24_REG_RX_ADDR_P0);
    for (i = 0; i < aw; i++) nRF24_printf("0x%02X ",buf[i]);
    nRF24_printf("\"\r\n");
    // RX_ADDR_P1
    nRF24_ReadMBReg(nRF24_REG_RX_ADDR_P1,buf,aw);
    nRF24_printf("[0x%02X] RX_ADDR_P1 \"",nRF24_REG_RX_ADDR_P1);
    for (i = 0; i < aw; i++) nRF24_printf("0x%02X ",buf[i]);
    nRF24_printf("\"\r\n");
    // RX_ADDR_P2
    nRF24_printf("[0x%02X] RX_ADDR_P2 \"",nRF24_REG_RX_ADDR_P2);
    for (i = 0; i < aw - 1; i++) nRF24_printf("0x%02X ",buf[i]);
    i = nRF24_ReadReg(nRF24_REG_RX_ADDR_P2);
    nRF24_printf("%c\"\r\n",i);
    // RX_ADDR_P3
    nRF24_printf("[0x%02X] RX_ADDR_P3 \"",nRF24_REG_RX_ADDR_P3);
    for (i = 0; i < aw - 1; i++) nRF24_printf("0x%02X ",buf[i]);
    i = nRF24_ReadReg(nRF24_REG_RX_ADDR_P3);
    nRF24_printf("%c\"\r\n",i);
    // RX_ADDR_P4
    nRF24_printf("[0x%02X] RX_ADDR_P4 \"",nRF24_REG_RX_ADDR_P4);
    for (i = 0; i < aw - 1; i++) nRF24_printf("0x%02X ",buf[i]);
    i = nRF24_ReadReg(nRF24_REG_RX_ADDR_P4);
    nRF24_printf("%c\"\r\n",i);
    // RX_ADDR_P5
    nRF24_printf("[0x%02X] RX_ADDR_P5 \"",nRF24_REG_RX_ADDR_P5);
    for (i = 0; i < aw - 1; i++) nRF24_printf("0x%02X ",buf[i]);
    i = nRF24_ReadReg(nRF24_REG_RX_ADDR_P5);
    nRF24_printf("%c\"\r\n",i);
    // TX_ADDR
    nRF24_ReadMBReg(nRF24_REG_TX_ADDR,buf,aw);
    nRF24_printf("[0x%02X] TX_ADDR \"",nRF24_REG_TX_ADDR);
    for (i = 0; i < aw; i++) nRF24_printf("0x%02X ",buf[i]);
    nRF24_printf("\"\r\n");
    // RX_PW_P0
    i = nRF24_ReadReg(nRF24_REG_RX_PW_P0);
    nRF24_printf("[0x%02X] RX_PW_P0=%u\r\n",nRF24_REG_RX_PW_P0,i);
    // RX_PW_P1
    i = nRF24_ReadReg(nRF24_REG_RX_PW_P1);
    nRF24_printf("[0x%02X] RX_PW_P1=%u\r\n",nRF24_REG_RX_PW_P1,i);
    // RX_PW_P2
    i = nRF24_ReadReg(nRF24_REG_RX_PW_P2);
    nRF24_printf("[0x%02X] RX_PW_P2=%u\r\n",nRF24_REG_RX_PW_P2,i);
    // RX_PW_P3
    i = nRF24_ReadReg(nRF24_REG_RX_PW_P3);
    nRF24_printf("[0x%02X] RX_PW_P3=%u\r\n",nRF24_REG_RX_PW_P3,i);
    // RX_PW_P4
    i = nRF24_ReadReg(nRF24_REG_RX_PW_P4);
    nRF24_printf("[0x%02X] RX_PW_P4=%u\r\n",nRF24_REG_RX_PW_P4,i);
    // RX_PW_P5
    i = nRF24_ReadReg(nRF24_REG_RX_PW_P5);
    nRF24_printf("[0x%02X] RX_PW_P5=%u\r\n",nRF24_REG_RX_PW_P5,i);
}
