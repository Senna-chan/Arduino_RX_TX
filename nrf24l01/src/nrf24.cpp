...
void nRF24_DumpConfig(void) {
	...
	for (i = 0; i < aw; i++) USART_printf(USART1,"0x%02X ",buf[i]);
	USART_printf(USART1,"\"\r\n");
	// RX_ADDR_P1
	...
	for (i = 0; i < aw - 1; i++) USART_printf(USART1,"0x%02X ",buf[i]);
...
void nRF24_SetCEPin(uint16_t pin)
{
	nRF_cePort = digitalPinToPort(pin);
	nRF_cePin = digitalPinToBitMask(pin);
}

void nRF24_SetCSNPin(uint16_t pin)
{
