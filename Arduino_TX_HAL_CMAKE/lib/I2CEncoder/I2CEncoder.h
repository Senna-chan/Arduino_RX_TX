...
#ifndef _I2C_ENCODER_H
#define _I2C_ENCODER_H
...
public:
	...
	uint8_t id = 0x00;
	typedef void (*Callback)(I2CEncoder*);
	...
	I2CEncoder(I2C_HandleTypeDef *hi2c, uint8_t add);
	...
	bool increased(){
		return readStatus(RINC);
	}
	bool decreased(){
		return readStatus(RDEC);
	}

	bool buttonPressed(){
		return readStatus(PUSHR);
	}

	bool buttonDoublePressed(){
		return readStatus(PUSHD);
	}
