...
extern int8_t USBD_CDC_TransmitComplete(uint8_t cdc_ch, uint8_t *Buf, uint32_t *Len, uint8_t epnum);
extern int8_t USBD_CDC_Receive(uint8_t cdc_ch, uint8_t *Buf, uint32_t *Len);
__weak int8_t USBD_CDC_TransmitComplete(uint8_t cdc_ch, uint8_t *Buf, uint32_t *Len, uint8_t epnum){}
__weak int8_t USBD_CDC_Receive(uint8_t cdc_ch, uint8_t *Buf, uint32_t *Len){}
...
static int8_t CDC_Receive(uint8_t cdc_ch, uint8_t *Buf, uint32_t *Len)
{
  ...
  USBD_CDC_SetRxBuffer(cdc_ch, &hUsbDevice, &Buf[0]);
    // USBSerial::getInstance(cdc_ch)->handleISR(BaseSerial::RX_EVENT, Buf, Len);
    USBD_CDC_Receive(cdc_ch, Buf, Len);
...
static int8_t CDC_TransmitCplt(uint8_t cdc_ch, uint8_t *Buf, uint32_t *Len, uint8_t epnum)
{
    USBD_CDC_TransmitComplete(cdc_ch, Buf, Len, epnum);
    // USBSerial::getInstance(cdc_ch)->handleISR(BaseSerial::RX_EVENT, Buf, Len);
  return (USBD_OK);
}
